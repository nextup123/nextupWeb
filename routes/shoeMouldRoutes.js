...
// Configuration
const YAML_PATH = '/home/nextup/user_config_files/planning_data/articles/articles.yaml';
const CURRENT_INDEX_YAML_PATH = '/home/nextup/user_config_files/planning_data/articles/article_current_index.yaml';
...
async function writeCurrentIndex(index) {
    const yamlContent = yaml.dump({ machine_current_index: index }, {
        indent: 2,
        lineWidth: -1,
        noRefs: true
    });
    ...
    try {
        ...
        if (articles !== undefined) {
            ...
            const mergedArticles = articles.map(newArticle => {
                const existing = currentData.articles.find(a => a.id === newArticle.id);
