// This file is automatically generated by rclnodejs
//
// *** DO NOT EDIT directly
//
'use strict';
const ref = require('../../third_party/ref-napi');
const StructType = require('@rclnodejs/ref-struct-di')(ref);
const ArrayType = require('@rclnodejs/ref-array-di')(ref);
const primitiveTypes = require('../../rosidl_gen/primitive_types.js');
const deallocator = require('../../rosidl_gen/deallocator.js');
const translator = require('../../rosidl_gen/message_translator.js');
const StringWrapper = require('../../generated/std_msgs/std_msgs__msg__String.js');
const PropertyRefStruct = StructType({
  name: primitiveTypes.string,
  description: primitiveTypes.string,
  type: primitiveTypes.string,
  value: primitiveTypes.string,
});
const PropertyRefArray = ArrayType(PropertyRefStruct);
const PropertyRefStructArray = StructType({
  data: PropertyRefArray,
  size: ref.types.size_t,
  capacity: ref.types.size_t
});
class PropertyWrapper {
  constructor(other) {
    this._initialize();
    this._setDefaults();
     if (typeof other === 'object' && other._refObject) {
      this._refObject = new PropertyRefStruct(other._refObject.toObject());
      this._wrapperFields.name =  new StringWrapper(other._wrapperFields.name);
      this._wrapperFields.description =  new StringWrapper(other._wrapperFields.description);
      this._wrapperFields.type =  new StringWrapper(other._wrapperFields.type);
      this._wrapperFields.value =  new StringWrapper(other._wrapperFields.value);
    } else if (typeof other !== 'undefined') {
      translator.constructFromPlanObject(this, other);
    }
    this.freeze(/*own=*/false);
  }
  _setDefaults() {
  }
  _initialize() {
    this._wrapperFields = {};
    this._refObject = new PropertyRefStruct();
    this._wrapperFields.name = new StringWrapper();
    this._wrapperFields.description = new StringWrapper();
    this._wrapperFields.type = new StringWrapper();
    this._wrapperFields.value = new StringWrapper();
  }
  static createFromRefObject(refObject) {
    let self = new PropertyWrapper();
    self.copyRefObject(refObject);
    return self;
  }
  static createArray() {
    return new PropertyArrayWrapper();
  }
  static get ArrayType() {
    return PropertyArrayWrapper;
  }
  static get refObjectArrayType() {
    return PropertyRefStructArray;
  }
  static get refObjectType() {
    return PropertyRefStruct;
  }
  toRawROS() {
    this.freeze(/*own=*/true);
    return this._refObject.ref();
  }
  freeze(own) {
    if (own) {
      this._wrapperFields.name.freeze(own);
    }
    this._refObject.name = this._wrapperFields.name.refObject;
    if (own) {
      this._wrapperFields.description.freeze(own);
    }
    this._refObject.description = this._wrapperFields.description.refObject;
    if (own) {
      this._wrapperFields.type.freeze(own);
    }
    this._refObject.type = this._wrapperFields.type.refObject;
    if (own) {
      this._wrapperFields.value.freeze(own);
    }
    this._refObject.value = this._wrapperFields.value.refObject;
  }
  serialize() {
    this.freeze(/*own=*/false);
    return this._refObject.ref();
  }
  deserialize(refObject) {
    this._wrapperFields.name.data = refObject.name.data;
    this._wrapperFields.description.data = refObject.description.data;
    this._wrapperFields.type.data = refObject.type.data;
    this._wrapperFields.value.data = refObject.value.data;
  }
  toPlainObject(enableTypedArray) {
    return translator.toPlainObject(this, enableTypedArray);
  }
  static freeStruct(refObject) {
    StringWrapper.freeStruct(refObject.name);
    StringWrapper.freeStruct(refObject.description);
    StringWrapper.freeStruct(refObject.type);
    StringWrapper.freeStruct(refObject.value);
  }
  static destroyRawROS(msg) {
    PropertyWrapper.freeStruct(msg.refObject);
  }
  static type() {
    return {pkgName: 'moveit_task_constructor_msgs', subFolder: 'msg', interfaceName: 'Property'};
  }
  static isPrimitive() {
    return false;
  }
  static get isROSArray() {
    return false;
  }
  get refObject() {
    return this._refObject;
  }
  get name() {
    return this._wrapperFields.name.data;
  }
  set name(value) {
    this._wrapperFields.name.data = value;
  }
  get description() {
    return this._wrapperFields.description.data;
  }
  set description(value) {
    this._wrapperFields.description.data = value;
  }
  get type() {
    return this._wrapperFields.type.data;
  }
  set type(value) {
    this._wrapperFields.type.data = value;
  }
  get value() {
    return this._wrapperFields.value.data;
  }
  set value(value) {
    this._wrapperFields.value.data = value;
  }
  copyRefObject(refObject) {
    this._refObject = new PropertyRefStruct(refObject.toObject());
    this._wrapperFields.name.copyRefObject(this._refObject.name);
    this._wrapperFields.description.copyRefObject(this._refObject.description);
    this._wrapperFields.type.copyRefObject(this._refObject.type);
    this._wrapperFields.value.copyRefObject(this._refObject.value);
  }
  copy(other) {
    this._refObject = new PropertyRefStruct(other._refObject.toObject());
    this._wrapperFields.name.copy(other._wrapperFields.name);
    this._wrapperFields.description.copy(other._wrapperFields.description);
    this._wrapperFields.type.copy(other._wrapperFields.type);
    this._wrapperFields.value.copy(other._wrapperFields.value);
  }
  static get classType() {
    return PropertyWrapper;
  }
  static get ROSMessageDef() {
    return {"constants":[],"fields":[{"name":"name","type":{"isArray":false,"arraySize":null,"isUpperBound":false,"isDynamicArray":false,"isFixedSizeArray":false,"pkgName":null,"type":"string","stringUpperBound":null,"isPrimitiveType":true},"default_value":null},{"name":"description","type":{"isArray":false,"arraySize":null,"isUpperBound":false,"isDynamicArray":false,"isFixedSizeArray":false,"pkgName":null,"type":"string","stringUpperBound":null,"isPrimitiveType":true},"default_value":null},{"name":"type","type":{"isArray":false,"arraySize":null,"isUpperBound":false,"isDynamicArray":false,"isFixedSizeArray":false,"pkgName":null,"type":"string","stringUpperBound":null,"isPrimitiveType":true},"default_value":null},{"name":"value","type":{"isArray":false,"arraySize":null,"isUpperBound":false,"isDynamicArray":false,"isFixedSizeArray":false,"pkgName":null,"type":"string","stringUpperBound":null,"isPrimitiveType":true},"default_value":null}],"baseType":{"pkgName":"moveit_task_constructor_msgs","type":"Property","stringUpperBound":null,"isPrimitiveType":false},"msgName":"Property"};
  }
  hasMember(name) {
    let memberNames = ["name","description","type","value"];
    return memberNames.indexOf(name) !== -1;
  }
}
class PropertyArrayWrapper {
  constructor(size = 0) {
    this._resize(size);
  }
  toRawROS() {
    return this._refObject.ref();
  }
  fill(values) {
    const length = values.length;
    this._resize(length);
    values.forEach((value, index) => {
      if (value instanceof PropertyWrapper) {
        this._wrappers[index].copy(value);
      } else {
        this._wrappers[index] = new PropertyWrapper(value);
      }
    });
  }
  freeze(own) {
    this._wrappers.forEach((wrapper, index) => {
      wrapper.freeze(own);
      this._refArray[index] = wrapper.refObject;
    });
    this._refObject.size = this._wrappers.length;
    this._refObject.capacity = this._wrappers.length;
    if (this._refObject.capacity === 0) {
      this._refObject.data = null
    } else {
      this._refObject.data = this._refArray.buffer;
    }
  }
  get refObject() {
    return this._refObject;
  }
  get data() {
    return this._wrappers;
  }
  get size() {
    return this._wrappers.length;
  }
  set size(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to PropertyArrayWrapper.size setter');
    }
    return this._resize(value);
  }
  get capacity() {
    return this._wrappers.length;
  }
  set capacity(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to PropertyArrayWrapper.capacity setter');
    }
    return this._resize(value);
  }
  _resize(size) {
    if (size < 0) {
      throw new RangeError('Invalid argument: should provide a positive number');
    }
    this._refArray = new PropertyRefArray(size);
    this._refObject = new PropertyRefStructArray();
    this._refObject.size = size;
    this._refObject.capacity = size;
    this._wrappers = new Array();
    for (let i = 0; i < size; i++) {
      this._wrappers.push(new PropertyWrapper());
    }
  }
  copyRefObject(refObject) {
    this._refObject = refObject;
    let refObjectArray = this._refObject.data;
    refObjectArray.length = this._refObject.size;
    this._resize(this._refObject.size);
    for (let index = 0; index < this._refObject.size; index++) {
      this._wrappers[index].copyRefObject(refObjectArray[index]);
    }
  }
  copy(other) {
    if (! (other instanceof PropertyArrayWrapper)) {
      throw new TypeError('Invalid argument: should provide "PropertyArrayWrapper".');
    }
    this._resize(other.size);
    other._wrappers.forEach((wrapper, index) => {
      this._wrappers[index].copy(wrapper);
    });
  }
  static freeArray(refObject) {
    let refObjectArray = refObject.data;
    refObjectArray.length = refObject.size;
    for (let index = 0; index < refObject.size; index++) {
      PropertyWrapper.freeStruct(refObjectArray[index]);
    }
  }
  static get elementType() {
    return PropertyWrapper;
  }
  static get isROSArray() {
    return true;
  }
  static get useTypedArray() {
    return false;
  }
  get classType() {
    return PropertyArrayWrapper;
  }
  toPlainObject(enableTypedArray) {
    return translator.toPlainObject(this, enableTypedArray);
  }
}
module.exports = PropertyWrapper;
