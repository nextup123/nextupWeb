...
function updateDriverStatus(data) {
  ...
  emergencyStatus.forEach((hasEmergency, i) => {
    ...
    const ok = !hasEmergency;
    led.classList.toggle("joint-indicator-true", ok);
    led.classList.toggle("joint-indicator-false", !ok);
  });
}
...
async function callStartServo() {
  const res = await fetch("/api/servo/start", { method: "POST" });
