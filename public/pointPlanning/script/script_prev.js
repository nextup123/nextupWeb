...
['j1', 'j2', 'j3', 'j4', 'j5', 'j6', 'cx', 'cy', 'cz', 'cr', 'cp', 'cw'].forEach(joint => {
    const plusButton = document.getElementById(`${joint}_plus`);
    const minusButton = document.getElementById(`${joint}_minus`);

    plusButton.addEventListener('pointerdown', () => {
        isPressed = true;
        ros2Publish(`+${joint}`);
        intervalId = setInterval(() => ros2Publish(`+${joint}`), 10);
        console.log(`Started +${joint}`);
    });
    ...
    plusButton.addEventListener('mouseleave', () => {
        if (isPressed) {
            ...
            ros2Publish(`0${joint}`);
            isPressed = false;
            console.log(`Stopped +${joint} (mouseleave)`);
        }
    });
...
function callStartServo() {
    ...
    startServoClient.callService(request, function (result) {
        ...
        button.style.backgroundColor = result.success ? 'var(--success)' : 'var(--danger)';
        ...
        showStatus(result.success ? 'Servo started' : 'Failed to start servo', result.success ? 'success' : 'error', 2000);
        setTimeout(() => {
            button.style.backgroundColor = '';
            button.style.background = 'linear-gradient(135deg, var(--success) 0%, #0a6b4a 100%)';
        }, 2000);
    });
}
            ...
            <div class="point-actions">
                <button class="btn-icon btn-move" 
                    onclick="showConfirmModal(
                        'Are you sure you want to move to ${point.name}?',
                        () => moveToPoint('get_last_pose@${point.name}')
                    )">
